...
`ranking_visibility = always_public` に変更後、再度STANDARD / TEXTをプレイし、以下を実ブラウザ確認済み。
...
- M2 / TT / m6 / P8 が各1回答・100%として表示される
- RECENT SESSIONS に 100 / 195 / 80 の3セッションが時刻付きで表示される
- 「この苦手を練習」導線が表示される

### 観察事項

テストデータが少ないため、現状の WEAK POINT が `TT / 三全音`（100%・1回答）と表示される。機能としては正常だが、サンプル数不足時に「苦手」と断定して見えるため、将来的には「判定保留 / データ不足」表示や最低回答数の閾値を設けるとより自然。

Phase 4の画面描画・オンライン履歴集計: PASS

次の実ブラウザ確認:

- 「この苦手を練習」押下時にFOCUS練習へ遷移すること
- TEXT / KEYS の回答方式選択が機能すること
- 選択した音程だけが練習対象になること
