...
create index if not exists rankings_user_idx
  on public.rankings (user_id);

alter table public.profiles enable row level security;
alter table public.rankings enable row level security;

-- 既存ポリシーがある場合も再実行できるように削除。
...
drop policy if exists "Rankings are visible to signed-in players" on public.rankings;
...
drop trigger if exists sync_interval_cosmos_profile_trigger on public.profiles;
