...
const tests=[
  ...
  ['admin final delete is service-role only',adminManagement.includes("auth.role(), '') <> 'service_role'")&&adminManagement.includes('grant execute on function public.admin_delete_player_application_row(uuid) to service_role')&&adminManagement.includes('from public, anon, authenticated')],
